...
Decide: recommend do, decline, defer or delegate; name human focus and review depth, a stopping condition, rationale and material uncertainty. Add reassessment trigger where useful. Protect obligations; name what substantial discretionary work displaces. Scout asks if a materially better approach exists; Multiplier asks if repeated friction merits a narrow intervention. Apply only when relevant. Do not create agents, queues or novelty backlogs.

Prepare: give necessary context, knowledge gaps, a contribution hypothesis and bounded preparation with a stopping rule. Ask only for inputs that could change the recommendation. AI inspection does not substitute for adequate human understanding; optional approved workers remain outside this system.
...
Evaluate a small real-case sample later using intended approach before advice and actual outcome, including harmful/uncertain attention changes. Agree an upkeep ceiling before real use and simplify if exceeded. Do not optimize throughput or enforce outcome logging.
